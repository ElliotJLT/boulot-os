...
// ─── Fit loop ─────────────────────────────────────────────────────────
//
// The expensive failure in this pipeline was never a crash. It was writing a CV,
// rendering it, discovering it ran to three pages, guessing what to cut, and
// re-rendering — repeatedly, with no idea whether the overshoot was forty words
// or four hundred. The model was negotiating with a black box.
//
// Everything below exists to replace that guess with a number. We measure the
// real laid-out document, in print media (which matters: `break-inside: avoid`
// pushes a whole role to the next page rather than splitting it, so raw content
// height under-reports), and report exactly which lines sit past the page budget.
//
...
const PRINTABLE_MM = PAGE_HEIGHT_MM - MARGIN_TOP_MM - MARGIN_BOTTOM_MM;

/**
 * Refuse to render a CV that has lost its header.
 *
 * The parser above skips the subtitle and contact lines when they don't match
 * its expected shape, without advancing its cursor — so a headline that isn't
 * wrapped in `**` causes the contact line to be tested as if it were the
 * headline, fail, and be discarded. That produced at least one real PDF with a
 * name and no phone number, email or LinkedIn, at exit code 0.
 *
 * A CV nobody can reply to is worse than no CV, so this is fatal, not a warning.
 */
function validateHeader(cv, markdown) {
  const problems = [];
  if (!cv.name) problems.push("no name (expected a line starting with `# `)");
  if (!cv.contactParts.length) problems.push("no contact line");

  // Losing the headline is cosmetic rather than fatal, but it is the same root
  // cause as losing the contact line — so say so before it becomes the latter.
  if (!problems.length && !cv.subtitle) {
    console.warn(
      "\n  ⚠ No headline parsed. The line under your name needs **double asterisks**.\n" +
        "    Rendering anyway, but the CV will have no positioning line.\n",
    );
  }
  ...
  console.error(`\n  ✗ Refusing to render: ${problems.join("; ")}.\n`);
  console.error("  The parser needs the first non-empty lines to look like:\n");
  console.error("      # Full Name");
  console.error("      **Headline goes here**            <- must be wrapped in **");
  console.error("      +44 ... | you@example.com | linkedin.com/in/you\n");
  console.error("  What it actually found:\n");
  preview.forEach((l) => console.error(`      ${l.slice(0, 78)}`));
  console.error(
    "\n  Most common cause: the headline is missing its ** wrapper, which makes\n" +
      "  the parser read the contact line as the headline and discard both.\n",
  );
  process.exit(2);
}
 ...
 *
 * This is ground truth — unlike a height estimate it accounts for Chrome's
 * actual pagination. Verified against `mdls` on every Chrome-produced CV in the
 * vault. Returns null for PDFs whose object streams are compressed (other
 * producers), in which case the caller falls back to the estimate and says so
 * rather than reporting a confident wrong number.
 */
function countPdfPages(path) {
  const buf = readFileSync(path).toString("latin1");
  const direct = (buf.match(/\/Type\s*\/Page[^s]/g) || []).length;
  if (direct > 0) return direct;
  const counts = (buf.match(/\/Count\s+(\d+)/g) || []).map((s) => +s.match(/\d+/)[0]);
  return counts.length ? Math.max(...counts) : null;
}

/**
 * Measure the laid-out document. Must run after emulateMediaType("print").
 *
 * Note the leaf selector covers every content type, not just bullets. A CV can
 * overflow because a role has too many bullets, but it can equally overflow
 * because Education, Skills and Languages trail off the bottom — and those
 * carry no <li> at all. An earlier version of this only looked at bullets and
 * confidently reported "0 characters over" on a document that was 140mm too
 * long, which is a worse failure than not measuring at all.
 */
async function measureLayout(page, budgetPx) {
  return page.evaluate((budget) => {
    const root = document.querySelector(".page") || document.body;
    const rootTop = root.getBoundingClientRect().top + window.scrollY;
    const topOf = (el) => el.getBoundingClientRect().top + window.scrollY - rootTop;
    const titleOf = (el) =>
      el.closest(".section")?.querySelector(".section-header")?.textContent?.trim() ?? "";

    const sections = [...document.querySelectorAll(".section")].map((el) => {
      const topPx = topOf(el);
      const heightPx = el.getBoundingClientRect().height;
      // Character density varies a lot between sections: a Skills block is
      // dense running text, an Experience bullet list is sparse (bullet
      // markers, leading, wrapping). Cutting N characters therefore reclaims
      // very different amounts of height depending on where you cut, so each
      // section carries its own chars-per-mm.
      const chars = (el.textContent ?? "").trim().replace(/\s+/g, " ").length;
      return {
        title: el.querySelector(".section-header")?.textContent?.trim() ?? "?",
        topPx,
        heightPx,
        chars,
        pastBudget: topPx >= budget,
        crossesBudget: topPx < budget && topPx + heightPx > budget,
      };
    });

    // Individual lines are the unit the model can act on: "cut these four
    // bullets" is executable, "the document is 62mm too long" is not.
    const LEAF = [
      ".bullets li",
      ".body-text",
      ".skill-item",
    ...
    const spill = [];
    for (const el of document.querySelectorAll(LEAF)) {
      const topPx = topOf(el);
      if (topPx < budget) continue;
      const text = (el.textContent ?? "").trim().replace(/\s+/g, " ");
      if (!text) continue;
      spill.push({
        section: titleOf(el),
        role: el.closest(".role")?.querySelector(".role-title")?.textContent?.trim() ?? "",
        chars: text.length,
        text,
      });
    }

    return { contentPx: root.scrollHeight, sections, spill };
  }, budgetPx);
}
...
function reportFit({ layout, actualPages, estimatedPages, maxPages, budgetPx, sidecarPath }) {
  ...
  const fits = pages <= maxPages;

  const sections = layout.sections.map((s) => {
    const heightMm = s.heightPx / PX_PER_MM;
    return {
      title: s.title,
      heightMm: Math.round(heightMm),
      chars: s.chars,
      charsPerMm: heightMm > 0 ? +(s.chars / heightMm).toFixed(1) : 0,
      pastBudget: s.pastBudget,
      crossesBudget: s.crossesBudget,
    };
  });

  // The cut target must be expressed in the currency of the section being cut.
  // Deriving it from the characters that happen to be spilling gives a wrong
  // answer whenever the spill and the bloat are in different sections, which is
  // the normal case: Experience is bloated, Education/Skills/Languages spill.
  //
  // The straight height-for-characters trade also under-delivers, because
  // `break-inside: avoid` means removing text does not shrink the document
  // smoothly. A role that no longer fits at the foot of page 2 is pushed whole
  // to page 3 and leaves whitespace behind, so some of what you cut buys back
  // nothing. Measured on a real 3-page CV, 142mm of removed text reclaimed
  // 99mm, about 70%. The margin below compensates. It is a heuristic, and the
  // report says so rather than promising a single perfect cut.
  const OVERFLOW_MARGIN = 1.4;
  const trimTarget = [...sections].sort((a, b) => b.heightMm - a.heightMm)[0];
  const charsToCut =
    trimTarget && trimTarget.charsPerMm > 0
      ? Math.round(overflowMm * trimTarget.charsPerMm * OVERFLOW_MARGIN)
      : spillChars;
  ...
  const report = {
    ...
    overflowMm: Math.round(overflowMm),
    charactersPastBudget: spillChars,
    trimTarget: trimTarget
      ? { section: trimTarget.title, charactersToCut: charsToCut, charsPerMm: trimTarget.charsPerMm }
      : null,
    sections,
    spill: layout.spill.map((s) => ({
      section: s.section,
      role: s.role,
      chars: s.chars,
  ...
  if (fits) {
    ...
    console.log(`  ✗ Over by ~${Math.round(overflowMm)}mm${spillChars ? ` — roughly ${spillChars} characters` : ""}.`);

    console.log("\n  Weight by section:");
    const totalMm = report.sections.reduce((n, s) => n + s.heightMm, 0) || 1;
    for (const s of report.sections) {
      const share = Math.round((s.heightMm / totalMm) * 100);
      const marker = s.pastBudget ? "  ← entirely past the mark" : s.crossesBudget ? "  ← crosses the mark" : "";
      console.log(
        `      ${String(s.heightMm).padStart(4)}mm  ${String(share).padStart(3)}%  ${String(s.charsPerMm).padStart(5)} ch/mm  ${s.title}${marker}`,
      );
    }

    if (report.spill.length) {
      console.log(`\n  ${report.spill.length} item(s) sit past the ${maxPages}-page mark:`);
      for (const s of report.spill) {
        const where = [s.role, s.section].filter(Boolean)[0] ?? "";
        console.log(
          `      [${String(s.chars).padStart(3)} chars] ${where ? `${where}: ` : ""}${s.text.slice(0, 68)}${s.text.length > 68 ? "…" : ""}`,
        );
      }
      // What spills is not necessarily what to cut. A CV usually overflows
      // because Experience is bloated, but what physically falls off the end is
      // Education/Skills/Languages: compact, high-signal sections you would
      // never delete. Give the budget in the currency of the section to cut.
      const t = report.trimTarget;
      console.log(
        `\n  → Cut ~${t.charactersToCut} characters from ${t.section}, then re-render.\n\n` +
          "  The items listed above are what currently falls past the mark. That is\n" +
          `  not what to cut: ${t.section} is the heaviest section, running at\n` +
          `  ${t.charsPerMm} characters per mm, so the cut is sized against its density.\n` +
          "  Cutting the same number of characters from a denser section reclaims\n" +
          "  much less height.\n\n" +
          "  The figure includes headroom. Whole roles cannot split across a page,\n" +
          "  so some of what you cut buys back whitespace rather than height, and a\n" +
          "  straight trade under-delivers. Expect this to clear it, or to leave a\n" +
          "  small remainder needing one more pass. Overflow shrinks every round.",
      );
    } else {
      // Overflow with nothing measurable past the line means it is spread
      // across the document rather than concentrated at the end.
      const biggest = [...report.sections].sort((a, b) => b.heightMm - a.heightMm)[0];
      console.log(
        `\n  Nothing sits wholly past the mark — the document is uniformly ~${Math.round(overflowMm)}mm\n` +
          `  too long. Largest section is ${biggest?.title} at ${biggest?.heightMm}mm (${Math.round(((biggest?.heightMm ?? 0) / totalMm) * 100)}%);\n` +
          "  trim there first.",
      );
    }
  }
  console.log(`  Machine-readable: ${sidecarPath}`);
  console.log("──────────────────────────────────────────────────────────\n");
...
const maxPagesArg = rawArgs.findIndex((a) => a === "--max-pages");
let maxPages = 2;
if (maxPagesArg !== -1) {
  maxPages = Number(rawArgs[maxPagesArg + 1]);
  if (!Number.isFinite(maxPages) || maxPages < 1) {
    console.error("--max-pages needs a positive number");
    process.exit(1);
  }
  rawArgs.splice(maxPagesArg, 2);
}

const args = rawArgs;
if (args.length < 1) {
  console.error("Usage: node generate-pdf.mjs <cv.md> [output.pdf] [--max-pages N]");
  process.exit(1);
}
...
validateHeader(cv, markdown);
console.log(`  Name: ${cv.name}`);
console.log(`  Subtitle: ${cv.subtitle}`);
console.log(`  Contact: ${cv.contactParts.join(" | ")}`);
console.log(
...
// Measure in PRINT media, not screen. The stylesheet carries @media print rules
// and break-inside/break-after directives; measuring on screen would report a
// layout the PDF never uses.
await page.emulateMediaType("print");
const budgetPx = maxPages * PRINTABLE_MM * PX_PER_MM;
const layout = await measureLayout(page, budgetPx);
...
const fits = reportFit({
  layout,
  actualPages: countPdfPages(outputPath),
  estimatedPages: Math.max(1, Math.ceil(layout.contentPx / (PRINTABLE_MM * PX_PER_MM))),
  maxPages,
  budgetPx,
...
// so it can be inspected; the non-zero code is what stops a too-long CV being
// treated as done.
if (!fits) process.exit(3);
