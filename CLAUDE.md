...
(On a brand-new copy these three files do not exist yet — that is the signal that
setup has not happened. Blank templates live in `USER/_template/`; `/start` copies
them into `USER/` and fills them in. Your real files are never tracked by git.)
...
## Always do first
1. **Check if this is a first-time user.** Run `ls USER/profile.md` — **if it does not exist, this is a first-time user.** (It is also a first-time user if the file exists but still reads as the blank template: placeholder prompts, empty fields, no real name.) Do NOT try to run a job search or read empty job folders. Instead, welcome them and run the onboarding in `/start`. Everything else depends on this being filled in.
